...
head(names(ossl.visnir), 20)
tail(names(ossl.visnir), 20)
...
head(names(ossl.mir), 20)
tail(names(ossl.mir), 20)

View(ossl.mir %>% select(!contains("_abs")))
...
names(ossl.soilsite)
# Remove dataset.doi_idf_c, dataset.code_ascii_txt, dataset.contact.email_ietf_email, location.error_any_m, location.address_utf8_txt
```
