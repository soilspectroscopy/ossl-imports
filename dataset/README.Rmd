...
```{r level1, include=TRUE, echo=TRUE, eval=TRUE}
...
Producing a golden dataset (that have complete observations for Spatial, VisNIR and MIR info).
...
### Map visualization
```{r golden_subset_map, include=TRUE, echo=TRUE, eval=TRUE}
...
This data processing follows the principles of `tidyverse` (@package_tidyverse) and takes advantage of several amazing packages, such as `tictoc` (@package_tictoc), `data.table` (@package_datatable), `lubridate` (@package_lubridate), `fs` (@package_fs), `qs` (@package_qs), `openssl` (@package_openssl), `olctools` (@package_olctools), `sf` (@package_sf), and `terra` (@package_terra).
