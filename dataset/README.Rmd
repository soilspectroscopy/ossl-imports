...
NOTE: The code chunk below this paragraph is hidden. Just run once for getting the OSSL column names, types, and example. Run once and upload to Google Sheet for formatting and integrating with the OSSL. Requires Google authentication.
...
# level0.soilsite %>%
#   filter(!is.na(pedon.taxa_usda_txt)) %>%
#   nrow()
...
NOTE: The code chunk below this paragraph is hidden. Run once for importing the harmonization rules (L0 to L1). The table can be edited online at Google Sheets after the previous upload. A copy is downloaded to github for archiving.
...
Reading soillab harmonization rules:
