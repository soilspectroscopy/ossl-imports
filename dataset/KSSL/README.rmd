...
author: "Tomislav Hengl (tom.hengl@opengeohub.org), Jonathan Sanderman (jsanderman@woodwellclimate.org), Develyn Bloom (develyn.bloom@ufl.edu), and Jose Lucas Safanelli (jsafanelli@woodwellclimate.org) - "
...
- `RaCA_measured.csv`: csv/rds file with averaged VNIR scans (from RaCA project);
...
packages <- c("tidyverse", "prospectr", "measurements", "readxl", "stringr",
              ...
              "googledrive", "googlesheets4", "data.table", "doMC")
...
This summary shows that, at total, about 92k observations are available. Some rows have both MIR and VisNIR scans, many not. As we have repeats for MIR (due to different preparations), it means that for the same layer id we can have different MIR scans, but similar VisNIR scans as they are repeated when joining. Therefore, when modeling specifically with the VisNIR spectra, it is necessary to drop or first those duplicates. They represent a minor fraction (n=60).
