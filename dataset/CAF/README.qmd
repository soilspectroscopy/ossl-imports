---
title: "CAF dataset preparation for the OSSL"
author: 
...
<!-- Additional documentation: [CAF.SSL](https://docs.soilspectroscopy.org/soilspec.html#caf.ssl) -->
...
packages <- c("tidyverse", "data.table", "lubridate", "units", "readxl", "stringr",
              "googledrive", "googlesheets4", "tmap", "sf", "skimr",
              "prospectr", "tictoc", "qs", "fs")
...
caf.sitedata <- caf.metadata %>%
  ...
  rename(id.layer_src_txt = sample_id,
         ...
         loc.alt_src_m = altitude,
         coord.long.point_wgs84_dd = gps_long,
         coord.lat.point_wgs84_dd = gps_lat,
         coord.err.point_any_m = gps_true,
  ...
  mutate(id.layer_src_txt = as.character(id.layer_original_txt)) %>%
  filter(lengths(str_split(sampling_layer, "-")) == 2) %>%
  separate(sampling_layer, into = c("layer.top.depth_any_cm", "layer.bottom.depth_any_cm"), sep = "-") %>%
  mutate(layer.seq_any_idx = ifelse(layer.top.depth_any_cm == 0, 1, 2),
         coord.err.point_any_m = ifelse(coord.err.point_any_m == "yes", 30, 1000)) %>%
  ...
  rename(date_iso_txt = sampling_date) %>%
...
NOTE: The code chunk below must be run just once. Run for getting the column standardization rules after editing online on Google Sheets. A copy of the output file is saved to this folder for archiving purposes.
