...
#       4536        1852        4239      117218       43927         304
...
## Produce final regression matrix:
gc()
...
sel.mir.name = c("id.scan_uuid_c", "id.layer_uuid_c", "mirmodel.code_any_c", paste0("scan_mir.", seq(600, 4000, by=2), "_abs"))
sel.vnir.name = c("id.scan_uuid_c", "id.layer_uuid_c", "vnirmodel.code_any_c", paste0("scan_visnir.", seq(350, 2500, by=2), "_pcnt"))
## takes minutes...
...
source("R-code/functions/SSL_functions.R")
summary(rm.ossl$layer.upper.depth_usda_cm)
## 830 NA + some very high values
## MIR only
ph.rm.mir.kssl = subset.ossl(rm.ossl, dataset.y="KSSL.SSL", geo.x=NULL, visnir.x=NULL)
dim(ph.rm.mir.kssl)
ph.rm.mir = subset.ossl(rm.ossl, geo.x=NULL, visnir.x=NULL)
dim(ph.rm.mir)
ph.rm.nir = subset.ossl(rm.ossl, geo.x=NULL, mir.x=NULL)
dim(ph.rm.nir)
ph.rm.mirnir = subset.ossl(rm.ossl, geo.x=NULL)
dim(ph.rm.mirnir)
ph.rm.all = subset.ossl(rm.ossl)
dim(ph.rm.all)

oc.rm = subset.ossl(ossl.x, tvar.y="oc_usda.calc_wpct")
